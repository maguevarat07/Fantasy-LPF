-- Daily production cadence; the queue wake-up still runs each minute for checkpoints.
create or replace function public.dispatch_fantasy_pipeline_message() returns bigint
...
begin
  ...
  if not found then
    if exists (select 1 from public.pipeline_runs
      where stage in ('PRICED','PARTIAL') and finished_at > now() - interval '24 hours') then
      return null;
    end if;
